...
$consulta="select * from smartfitness.entrenador where documento='$documento'";
...
if($row=mysql_fetch_array($result))
{
	...
	echo'
		...
		<form method="post" autocomplete="off" action="INSERTAR_ACTUALIZACION_ENTRENADOR.php" onsubmit="return validar(this);" >

		<legend> ACTUALIZAR DATOS ENTRENADOR </legend>
			...
			<input type="text" name="documento" value="'.$row["documento"].'" size="29" pattern="[0-9]+" title="El documento debe estar formado únicamente por numeros" />
			</div>
            
            <div>
			<label for="pass">EDAD:</label>
			<input type="text" name="edad" value="'.$row["edad"].'" size="29" pattern="[0-9]+" title="La edad debe estar formado únicamente por numeros"/>
			</div>
			...
			<input type="text" name="mail" value="'.$row["mail"].'" size="29" pattern="^[a-zA-Z0-9.!#$%&*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$" title="El corre debe estar formado correctamente" />
			</div>
            
            <div>
			<label for="pass">Celular:</label>
			<input type="text" name="celu" value="'.$row["celular"].'" pattern="[0-9]+" title="El celular debe estar formado únicamente por numeros" size="29"/>
			</div>
			
			<div>
			<label for="pass">Telefono:</label>
			<input type="text" name="tele" value="'.$row["telefono"].'" pattern="[0-9]+" title="El telefono debe estar formado únicamente por numeros" size="29"/>
			</div>
            
            <div>
			<label for="pass">Usuario:</label>
			<input type="text" name="login" value="'.$row["login"].'" pattern="|^[a-zA-Z ñÑáéíóúüç]*$|" title="El usuario debe estar formado únicamente por letras" size="29"/>
			</div>
			
			<div>
			<label for="pass">Contraseña:</label>
			<input type="text" name="contrasena" value="'.$row["contrasena"].'" pattern="\S{8,10}"  title="La contraseña debe ser mayor de 8 numeros" size="29"/>
			</div>
            
            <div>
			<label for="pass">Horario De Trabajo:</label>
			<input type="text" name="horario" value="'.$row["horario"].'"  title="El horario debe estar formado únicamente por letras y numeros" size="29"/>
			</div>
         ';
